...
test("forces new users to student and keeps secrets out of the login UI",async()=>{const users=await read("app/lib/database/users.ts");assert.match(users,/role:"student"/);const login=await read("app/login/page.tsx");assert.doesNotMatch(login,/SERVICE_ROLE|CLIENT_SECRET|access_token|refresh_token|process\.env/)});
