...
export default function ClubsPage(){
 ...
 return <main className="app-shell clubs-page">
  ...
  <section className="clubs-explorer">
   ...
   <div className="clubs-filters" role="group" aria-label="분야 빠른 필터">{categories.map(item=><button type="button" key={item} className={category===item?"active":""} aria-pressed={category===item} onClick={()=>setCategory(item)}>{item}</button>)}</div>
