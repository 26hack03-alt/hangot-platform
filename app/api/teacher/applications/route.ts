import { countApplications, listApplications, sanitizeApplicationSearch } from "../../../lib/database/applications";
...
export async function GET(request: Request) {
  ...
  const params = new URL(request.url).searchParams, page = positive(params.get("page"), 1), pageSize = Math.min(100, positive(params.get("pageSize"), 20)), status = params.get("status") ?? "", clubId = sanitizeText(params.get("clubId"), 80), search = sanitizeApplicationSearch(params.get("search") ?? ""), sort = params.get("sort") === "oldest" ? "oldest" : "newest";
  ...
  const options = { status: isApplicationStatus(status) ? status : undefined, search: search || undefined, searchUserIds, page, pageSize, sort, clubIds: clubs } as const;
