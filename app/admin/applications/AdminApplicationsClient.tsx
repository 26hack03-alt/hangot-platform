...
const initialFilters={status:"",clubId:"",search:"",sort:"newest"};
...
export default function AdminApplicationsClient(){
  ...
  const[filters,setFilters]=useState(initialFilters); const[applied,setApplied]=useState(initialFilters); const[page,setPage]=useState(1);
  ...
  return <PortalShell title="신청 관리" description="전체 동아리 신청을 조회하고 검토 상태를 관리합니다.">
    ...
    <form className="panel admin-filters" onSubmit={search}><label>상태<select value={filters.status} onChange={e=>setFilters({...filters,status:e.target.value})}><option value="">전체 상태</option>{Object.entries(labels).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label><label>동아리<select value={filters.clubId} onChange={e=>setFilters({...filters,clubId:e.target.value})}><option value="">전체 동아리</option>{clubs.map(c=><option value={c.club_id} key={c.club_id}>{c.club_name}</option>)}</select></label><label>검색<input maxLength={200} placeholder="학번, 이름, 신청 번호 또는 지원 동기" value={filters.search} onChange={e=>setFilters({...filters,search:e.target.value})}/></label><label>정렬<select value={filters.sort} onChange={e=>setFilters({...filters,sort:e.target.value})}><option value="newest">최신순</option><option value="oldest">오래된순</option></select></label><div className="admin-filter-actions"><button className="primary" type="submit">조회</button><button className="secondary" type="button" onClick={reset}>초기화</button></div></form>
    {error?<div className="empty-panel">{error}</div>:!data?<div className="empty-panel">신청 목록을 불러오는 중입니다.</div>:data.applications.length===0?<div className="empty-panel">조건에 맞는 신청이 없습니다.</div>:<div className="list-stack">{data.applications.map(item=><article className="list-card admin-application-card" key={item.id}><div className="admin-applicant"><h3>{item.studentNumber && item.studentName ? `${item.studentNumber} ${item.studentName}` : "학생 정보 미등록"}</h3><p className="applicant-alias">{item.userAlias}</p><p className="applicant-club">{item.clubName}</p><small>신청번호 {item.applicationNumber}</small><p>제출 {new Date(item.submittedAt).toLocaleString("ko-KR")}</p></div><div className="admin-application-actions"><span className={`state state-${item.status}`}>{labels[item.status]??item.status}</span><Link className="secondary link-button" href={`/admin/applications/${item.id}`}>상세 보기</Link></div></article>)}</div>}
