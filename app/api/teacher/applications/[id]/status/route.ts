...
import { canReviewTransition, isApplicationStatus } from "../../../../../lib/application-admin";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const limited = await checkRateLimit(request, { scope: "teacher-application-review", identifier: auth.user.id, limit: 30, windowMs: 60_000 });
  ...
  if (!current || !(await teacherClubIds(auth.user.id)).includes(current.club_id)) return Response.json({ error: "NOT_FOUND" }, { status: 404 });
  if (!canReviewTransition(current.status, body.status)) return Response.json({ error: "INVALID_STATUS_TRANSITION" }, { status: 409 });
  const comment = sanitizeText(body.reviewComment, 1000) || null;
  try {
    await reviewApplication({ applicationId, expectedStatus: current.status, nextStatus: body.status, reviewComment: comment, actorUserId: auth.user.id, actorRole: "teacher" });
  } catch {
