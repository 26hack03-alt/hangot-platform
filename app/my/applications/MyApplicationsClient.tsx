  ...
  return <PortalShell title="내 신청" description="신청 내역과 진행 상태를 확인할 수 있습니다.">
    ...
    {items === null ? <div className="empty-panel">신청 내역을 불러오는 중입니다.</div> : items.length === 0
      ...
      : <div className="list-stack">{items.map((item) => <article className="list-card" key={item.id}>
          ...
          <div><span className={`state state-${item.status}`}>{labels[item.status] ?? item.status}</span><Link href={`/my/applications/${item.id}`}>상세 보기</Link>{cancellable.has(item.status) && <button className="text-danger" onClick={() => cancel(item.id)}>신청 취소</button>}</div>
        </article>)}</div>}
  </PortalShell>;
}
