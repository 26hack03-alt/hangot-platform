  ...
  return <PortalShell title={created ? "신청 완료" : "신청 상세"} description={created ? "동아리 신청이 정상적으로 접수되었습니다." : "제출한 신청 내용과 상태를 확인할 수 있습니다."}>
    {notFound ? <div className="empty-panel"><p>신청 내역을 찾을 수 없습니다.</p><Link href="/my/applications">내 신청 목록으로</Link></div> : !application
      ...
      : <div className="panel application-detail">
        <div className="application-detail-heading"><div><small>신청 번호</small><h2>{application.applicationNumber}</h2><p>{application.clubName}</p></div><span className={`state state-${application.status}`}>{labels[application.status] ?? "상태 확인 필요"}</span></div>
        ...
        <div className="button-row"><Link className="secondary link-button" href="/my/applications">내 신청 내역으로 이동</Link><Link className="secondary link-button" href="/">동아리 목록으로 이동</Link>{cancellable.has(application.status) && <button className="text-danger" type="button" disabled={cancelling} onClick={cancel}>{cancelling ? "취소 중…" : "신청 취소"}</button>}</div>
      </div>}
  </PortalShell>;
}
